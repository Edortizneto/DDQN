...
class DoubleDeepQLearning:
    ...
    def __init__(self, env, gamma, epsilon, epsilon_min, epsilon_dec, episodes, batch_size, memory, model):
        ...
        self.memory = memory
        self.model = model
        self.secundary_model = keras.models.clone_model(model)
        self.secundary_model.set_weights(model.get_weights())

    def select_action(self, state):
        if np.random.rand() < self.epsilon:
            return random.randrange(self.env.action_space.n)
        action = self.secundary_model.predict(state,verbose=0)
    ...
    def train(self):
        ...
        for i in range(self.episodes+1):
            ...
            #while not terminal:
                ...
                if terminal:
                    print(f'Episódio: {i}/{self.episodes}. Score: {score}')
                    break
            rewards.append(score)
            if i % refresh == 0:
                self.secundary_model.set_weights(self.model.get_weights())
                print(f"Sec. Model Att \n last {refresh} mean: {np.mean(rewards[-refresh:])}")
